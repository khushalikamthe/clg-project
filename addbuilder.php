...
//If supervisior clicked register button
if(isset($_POST)) {
	...
	$sql = "SELECT email FROM builder WHERE email='$email'";
	...
	if($result->num_rows == 0) {
		...
		if($uploadOk == false) {
			header("Location: register-builder.php");
			exit();
		}

		//sql new registration insert query
		$sql = "INSERT INTO builder(name, companyname, country, state, city, contactno, website, email, password, aboutme, logo) VALUES ('$name', '$companyname', '$country', '$state', '$city', '$contactno', '$website', '$email', '$password', '$aboutme', '$file')";

		if($conn->query($sql)===TRUE) {

			//If data inserted successfully then Set some session variables for easy reference and redirect to builder login
			$_SESSION['registerCompleted'] = true;
			header("Location: login-builder.php");
			exit();
